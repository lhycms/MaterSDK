...
void LmpDeepPotSe::allocate() {
    // Step 1. Populate `this->allocated`
    this->allocated = 1;

    // Step 2. Populate `this->setflag`
    int num_types = this->atom->ntypes;
    this->memory->create<int>(this->setflag, num_types+1, num_types+1, "LmpDeepPotSe:setflag");
    ...
    this->memory->create<double>(this->relative_cart_coords, num_atoms, 3, "LmpDeepPotSe:relative_cart_coord");
    this->memory->create<double>(this->tilde_r, num_atoms, max_num_neigh_atoms, 4, "LmpDeepPotSe:tilde_r");
    this->memory->create<double>(this->tilde_r_deriv, num_atoms, max_num_neigh_atoms, 4, 3, "LmpDeepPotSe:tilde_r_deriv");
}


void LmpDeepPotSe::settings(int argc, char** argv) {
    
}


double*** LmpDeepPotSe::get_neigh_relative_cart_coords() const {
    ...
    this->memory->create<double>(center_cart_coord, 3, "LmpDeepPotSe:center_cart_coord");
    double* neigh_cart_coord;
    this->memory->create<double>(neigh_cart_coord, 3, "LmpDeepPotSe:neigh_cart_coord");
